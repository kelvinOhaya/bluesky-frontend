...
function ChatRoomProvider({ children }) {
  ...
  const loadChatRooms = async () => {
    try {
      console.log("loadChatRooms: Making API call to /chatroom/send-info");
      const { data } = await api.get("/chatroom/send-info");
      console.log("loadChatRooms: Received data:", data);
      ...
      console.log(
        "loadChatRooms: Set chatRooms, count:",
        data.chatRooms.length
      );

      // If currentChat is null, default to the welcome chat or first available chat
      if (data.currentChat && data.currentChat._id) {
        console.log(
          "loadChatRooms: Setting currentChatId to:",
          data.currentChat._id
        );
        setCurrentChatId(data.currentChat._id);
      } else if (data.chatRooms.length > 0) {
        console.log("loadChatRooms: No currentChat, finding default...");
        // Find welcome chat (ID: 68c0671711b70a88b9b0cd90) or use first chat
        const welcomeChat = data.chatRooms.find(
          (room) => room._id === "68c0671711b70a88b9b0cd90"
        );
        const defaultChat = welcomeChat || data.chatRooms[0];
        console.log("loadChatRooms: Using default chat:", defaultChat._id);
        setCurrentChatId(defaultChat._id);

        // Update the user's currentChat in the database
        await api.put("/chatroom/update-current-room", {
          currentRoomId: defaultChat._id,
          socketId: socket?.id,
        });
      }
    } catch (error) {
      console.error("loadChatRooms: Error occurred:", error);
      console.error("loadChatRooms: Error response:", error.response);
  ...
  useEffect(() => {
    const getLoadedMessages = async () => {
      ...
      if (!currentChat) {
        console.log("No currentChat, skipping loadMessages");
        return;
      }
      await loadMessages();
    };
